// src/modules/entrevistas/components/tabs/HistorialLaboralTab.tsx
...
export default function HistorialLaboralTab({ entrevistaId, data, onSaved }: Props) {
  ...
  // --- ESTADO DE LA LISTA DE EXPERIENCIAS ---
  ...
  return (
    <div className="animate-fade-in" style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <Card size="small" title={<Text style={{ fontSize: 13, color: '#8b949e' }}>Antecedentes Generales</Text>}>
        <Row gutter={[16, 16]}>
          <Col span={24} md={12}>
            <Text className="block mb-1 text-xs text-gray-400 uppercase">Se enteró de esta vacante por medio de:</Text>
            <Select className="w-full" placeholder="Seleccione medio" value={medioVacante} onChange={setMedioVacante} size="small">
              {opcionesVacante.map(opt => <Select.Option key={opt} value={opt}>{opt}</Select.Option>)}
            </Select>
          </Col>
          <Col span={24} md={12}>
            <Text className="block mb-1 text-xs text-gray-400 uppercase">¿Cometió usted algún acto ilícito en alguno de sus trabajos?</Text>
          ...
          <Col span={24}>
            <Text className="block mb-1 text-xs text-gray-400 uppercase">¿Qué es lo más grave que usted ha realizado en sus trabajos anteriores?</Text>
            ...
            {expandedKey === t.key && (
              <div style={{ padding: '16px', borderTop: '1px solid #21262d' }}>
                <Row gutter={[16, 0]}>
                  ...
                  <Col xs={24} md={4}><Text style={{ color: '#8b949e', fontSize: 12, display: 'block', marginBottom: 4 }}>Salario ($)</Text><Input type="number" value={t.salario} onChange={(e) => update(t.key, 'salario', e.target.value ? Number(e.target.value) : undefined)} placeholder="0.00" prefix="$" style={{ marginBottom: 12 }} /></Col>
                  <Col xs={24} md={4}><Text style={{ color: '#8b949e', fontSize: 12, display: 'block', marginBottom: 4 }}>¿Trabajo actual?</Text><Switch checked={t.trabajo_actual} onChange={(v) => { update(t.key, 'trabajo_actual', v); if (v) update(t.key, 'fecha_fin', null); }} checkedChildren="Sí" unCheckedChildren="No" style={{ marginBottom: 12 }} /></Col>
                  ...
                  <Col xs={24} md={8}><Text style={{ color: '#8b949e', fontSize: 12, display: 'block', marginBottom: 4 }}>¿Cuenta con certificado laboral?</Text><Switch checked={t.certificado_laboral} onChange={(v) => update(t.key, 'certificado_laboral', v)} checkedChildren="Sí" unCheckedChildren="No" style={{ marginBottom: 12 }} /></Col>
                </Row>
                
                {!t.trabajo_actual && (
                  <>
                    <Text style={{ color: '#8b949e', fontSize: 12, display: 'block', marginBottom: 4, marginTop: 4 }}>Motivo de Salida</Text>
      ...
      {/* SECCIÓN 3: OBSERVACIONES DEL ENTREVISTADOR */}
      <Card size="small">
        <Text className="block mb-1 text-xs text-gray-400 uppercase">Observaciones o Verificaciones del Entrevistador</Text>
