...
export default function ValidacionesTab({ entrevistaId, data, onSaved }: Props) {
  ...
  const { control, handleSubmit, reset, watch, formState: { isSubmitting } } = useForm({
    defaultValues: { documentos_verificados: false, referencias_verificadas: false, resultado_general: 'PENDIENTE', calificacion: 70 },
  });

  useEffect(() => { if (data) reset({ ...data, calificacion: data.calificacion ?? 70 }); }, [data]);

  const resultado     = watch('resultado_general') as string;
  ...
  const onSubmit = async (values: any) => {
    try {
      await entrevistasApi.saveValidaciones(entrevistaId, { ...values, fecha_validacion: new Date().toISOString() });
      message.success('Validaciones finales guardadas. Entrevista completada.');
      onSaved();
    } catch { message.error('Error al guardar validaciones'); }
  };
          ...
          <Form.Item label={lbl(`Calificación: ${calificacion}/100`)}>
            <Controller name="calificacion" control={control} render={({ field }) => (
              <Slider {...field} min={0} max={100} step={1}
                trackStyle={{ background: getCalColor(calificacion as number) }}
                handleStyle={{ borderColor: getCalColor(calificacion as number) }} />
            )} />
          </Form.Item>
          <div style={{ textAlign: 'center', marginTop: -12 }}>
            <Text style={{ fontSize: 32, fontWeight: 700, color: getCalColor(calificacion as number) }}>{calificacion}</Text>
